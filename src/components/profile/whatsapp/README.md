...
- **La plantilla de inicio se elige a mano, siempre** (`WhatsAppStartTemplateModal`). El botón "Iniciar conversación" abre un selector con las plantillas **aprobadas** de la cuenta, leídas en vivo de Meta (`companyWhatsappTemplates`), con sus variables y una vista previa de cómo le va a llegar. No se guarda un catálogo local a propósito: el estado de una plantilla cambia del lado de Meta y una copia se desincronizaría igual que pasó con `whatsappTemplateStatus`. Antes se mandaba siempre la plantilla de leads, que en un chat interno con el equipo sonaba a mensaje de ventas — de ahí el cambio. Como propuesta, `{{1}}` se prellena con el destinatario y `{{2}}` con la empresa, pero ambos son editables.
- **El estado de la plantilla se relee de Meta en cada prueba de conexión**, no solo por webhook. El webhook (`message_template_status_update`) es un paso manual que mucha gente no completa, y sin él la plantilla se queda en "pendiente de aprobación" para siempre aunque Meta ya la haya aprobado — que era exactamente el síntoma reportado. Por eso "Probar conexión" también sirve como "refrescar estado": si ves aprobada en WhatsApp Manager y pendiente aquí, es el botón que lo arregla. Si la plantilla ya no existe en Meta, la prueba la vuelve a crear.

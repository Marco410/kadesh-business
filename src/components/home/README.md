...
Copy alineado con `src/components/profile/ai/README.md`: marca `Kadesh AI`; no proveedores, modelos, ni “inyectar contexto”. CTA a registro. El admin configura; el equipo lo ve en Inicio. **Está en todos los planes.** El chip del hero, el ítem del menú y el badge de diferenciadores usan el degradado violeta–azul (`--ai-urim-purple` / `--ai-urim-blue`, clase `ai-urim-fill`).
